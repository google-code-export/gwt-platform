...
import com.gwtplatform.mvp.client.annotations.TabInfoGenerator;
...
public class ProxyGenerator extends Generator {
  ...
  private static final String requestTabsHandlerGenericClassName = RequestTabsHandlerGeneric.class.getCanonicalName();
  private JClassType requestTabsHandlerGenericClass = null;  
  ...
  private static final String tabContentProxyGenericClassName = TabContentProxyGeneric.class.getCanonicalName();
  private JClassType tabContentProxyGenericClass = null;
  ...
  private static final String tabDescriptionTextClassName = TabDescriptionText.class.getCanonicalName();
  ...
  throws UnableToCompleteException {
    ...
    if( proxyInterface.isAssignableTo( tabContentProxyGenericClass ) ) {
      
      // TODO This is a hack because JClassType doesn't let us access the list of all annotations.
      //      As a consequence, we have to annotate the proxy itself with @TabInfoGenerator.
      //      In truth, it should be possible to only annotate the @TabInfo-like annotation.
      TabInfoGenerator tabInfoGeneratorAnnotation = proxyInterface.getAnnotation(TabInfoGenerator.class);
      TabDescriptionGenerator tabDescriptionGenerator;
      
      // TODO Continuation of previous hack. @TabInfo is a GWTP "internal" annotation. If it is present, we
      //      can infer that the generator is TabDescriptionTextGenerator.
      if( tabInfoGeneratorAnnotation == null ) {
        if(  proxyInterface.isAnnotationPresent(TabInfo.class) ) {
          tabDescriptionGenerator = new TabDescriptionTextGenerator();
        } else {
          logger.log(TreeLogger.ERROR, "The proxy for '" + presenterClassName + "' is a TabContentProxy, but is not annotated with @" +
              TabInfo.class.getSimpleName() + " or @" + TabInfoGenerator.class.getSimpleName() + ".", null);
          throw new UnableToCompleteException();          
        }
      } else {
        try {
          tabDescriptionGenerator = tabInfoGeneratorAnnotation.value().newInstance();
        } catch (Throwable e) {
          logger.log(TreeLogger.ERROR, "While generating proxy for '" + presenterClassName + "' an exception occured during instantiation of '" +
              tabInfoGeneratorAnnotation.value().getSimpleName() + "'.", e);          
          throw new UnableToCompleteException();          
        }
      }
    ...
    if( nameToken == null ) {
      ...
      if( tabContainerClass == null ) {
        ...
        composerFactory.setSuperclass(tabContentProxyImplClassName+"<"+tabDescriptionTextClassName+","+presenterClassName+">" );
      }
    }
    else {
      // A place
      if( tabContainerClass == null ) {
        ...
        composerFactory.setSuperclass(tabContentProxyPlaceImplClassName+"<"+tabDescriptionTextClassName+","+presenterClassName+">" );
      }
    }
    ...
    if( nameToken != null ) {
      ...
      if( tabContainerClass == null )
        ...
        writer.println( "extends " +tabContentProxyImplClassName+"<"+tabDescriptionTextClassName+","+presenterClassName+"> {" );
      writer.indent();
  ...
  throws UnableToCompleteException {
    ...
    for( JField field : tabContainerClass.getFields() ) {
      ...
      if( annotation != null ) {
        if( !field.isStatic() || 
            ...
            !parameterizedType.getTypeArgs()[0].isAssignableTo(requestTabsHandlerGenericClass) ) {
    ...
    writer.print( "tabDescription = new " + tabDescriptionTextClassName + "( " );
    // 1st param is label
    if( tabLabel != null )
      writer.print( "\"" + tabLabel + "\", " );
    else
      writer.print( tabGetLabel + "," );
    // 2nd param is history token
    writer.print( "\"" + nameToken + "\", " );
    // 3rd param is priority  
  ...
  private void findBaseTypes( GeneratorContext ctx ) {
    ...
    requestTabsHandlerGenericClass = oracle.findType( requestTabsHandlerGenericClassName );
    ...
    tabContentProxyGenericClass = oracle.findType( tabContentProxyGenericClassName );
