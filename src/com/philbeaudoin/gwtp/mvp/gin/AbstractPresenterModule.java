...
public abstract class AbstractPresenterModule extends AbstractGinModule {
  ...
  /**
   * Convenience method for binding a non-singleton {@link PresenterWidgetImpl}
   * with its {@link View}.
   * 
   * @param <P>
   *          The {@link PresenterWidgetImpl} type.
   * @param <V>
   *          The {@link View} type.
   * @param presenter
   *          The {@link PresenterWidgetImpl} (NOT a singleton).
   * @param viewImpl
   *          The {@link View} implementation (NOT a singleton).
   */
  @SuppressWarnings("unchecked")
  protected <V extends View, P extends PresenterWidgetImpl<? super V>> void autoBindPresenterWidget(
   ...
   * proxy, when using automatically generated proxy classes. This method use
   * reflection to find the proxy and the view classes.
   * 
   * @param <P>
   *          The {@link PresenterImpl} type.
   * @param <V>
   *          The {@link View} type.
   * @param presenter
   *          The {@link PresenterImpl} (a singleton).
   * @param viewImpl
   *          The {@link View} implementation (a singleton).
   * 
   * @see #bindPresenter(Class, Class, Class, Class)
   */
  @SuppressWarnings("unchecked")
  protected <V extends View, P extends PresenterImpl<? super V, ?>> void autoBindPresenter(
      Class<P> presenter, Class<V> viewImpl) {
    Type[] args = ReflectionUtils.getTypeArguments(PresenterImpl.class,
        presenter);
    bindPresenter(presenter, (Class<View>) args[0], viewImpl,
        (Class<Proxy<P>>) args[1]);
  }
   ...
   * 
   * @param <P>
   *          The {@link PresenterWidget} type.
   * @param <V>
   *          The {@link View} type.
   * @param presenter
   *          The {@link PresenterWidget} (NOT a singleton).
   * @param view
   *          The {@link View} interface.
   * @param viewImpl
   *          The {@link View} implementation (NOT a singleton).
   */
  protected <P extends PresenterWidget, V extends View> void bindPresenterWidget(
      Class<P> presenter, Class<V> view, Class<? extends V> viewImpl) {
    bind(presenter);
   ...
   * 
   * @param <P>
   *          The type of the {@link PresenterWidget} factory.
   * @param <V>
   *          The type of the {@link View} factory.
   * @param presenterFactory
   *          The interface to the {@link PresenterWidget} factory.
   * @param presenterFactoryImpl
   *          The implementation of the {@link PresenterWidget} factory.
   * @param viewFactory
   *          The interface to the {@link View} factory.
   * @param viewFactoryImpl
   *          The implementation of the {@link View} factory.
   */
   ...
   *          The {@link View} type.
   * @param presenter
   *          The {@link PresenterWidget} (a singleton).
   * @param view
   *          The {@link View} interface.
   * @param viewImpl
   *          The {@link View} implementation (a singleton).
   ...
   *          The {@link Proxy} type.
   * @param presenter
   *          The {@link Presenter} (a singleton).
   * @param view
   *          The {@link View} interface.
   * @param viewImpl
   *          The {@link View} implementation (a singleton).
   * @param proxy
   *          The {@link Proxy} interface, which will lead to an automatically
   ...
   * @param proxyImpl
   *          The {@link Proxy} implementation (a singleton).
   * 
   * @see #bindPresenter(Class, Class, Class, Class)
   */
  protected <P extends Presenter, V extends View, Proxy_ extends Proxy<P>> void bindPresenter(
  ...
  protected <P extends Presenter, Proxy_ extends Proxy<P>> void bindPresenter(
      Class<P> presenter, Class<Proxy_> proxy) {
    bind(presenter).in(Singleton.class);
    bind(proxy).asEagerSingleton();
  }
   ...
   */
  protected <P extends Presenter, Proxy_ extends Proxy<P>> void bindPresenter(
      Class<P> presenter, Class<Proxy_> proxy, Class<? extends Proxy_> proxyImpl) {
    bind(presenter).in(Singleton.class);
    bind(proxyImpl).asEagerSingleton();
    bind(proxy).to(proxyImpl);
  }
   ...
   * {@link #bindPresenter(Class, Class)} or
   * {@link #bindPresenter(Class, Class, Class)}. If the view class is use only
   * by one presenter, you should consider using
   * {@link #bindPresenter(Class, Class, Class, Class, Class)} instead.
   * 
   * @param <V>
   *          The {@link View} type.
